...
decision_space_canvas = Canvas(main_frame, width=ancho_canva, height=alto_canva, bg='white', highlightthickness=1, highlightbackground="black")
...
decision_space_canvas.create_line(padding, alto_canva - padding, ancho_canva - 10, alto_canva - padding, arrow="last")  # Eje X
decision_space_canvas.create_line(padding, alto_canva - padding, padding, 10, arrow="last")  # Eje Y
...
decision_space_canvas.create_text(10, 10, text="Espacio de Decisión (x, y)", anchor="nw")


objective_space_canvas = Canvas(main_frame, width=ancho_canva, height=alto_canva, bg='ivory', highlightthickness=1, highlightbackground="black")
...
def graficar_frente_pareto(canvas, lideres, ancho, alto):
    ...
    lideres_validos = [(x, y, obj) for x, y, obj in lideres if all(not isinstance(v, complex) and v != float('inf') for v in obj)]
    ...
    canvas.create_line(padding, alto - padding, ancho - 10, alto - padding, arrow="last")
    canvas.create_text(ancho / 2, alto - 15, text="Objetivo 1 (f1)")
    canvas.create_text(padding, alto - padding + 10, text=f"{f1_range[0]:.2f}", anchor="w")
    canvas.create_text(ancho - 10, alto - padding + 10, text=f"{f1_range[1]:.2f}", anchor="e")
    
    canvas.create_line(padding, alto - padding, padding, 10, arrow="last")
    canvas.create_text(15, alto / 2, text="Objetivo 2 (f2)", angle=90)
    canvas.create_text(padding - 10, alto - padding, text=f"{f2_range[0]:.2f}", anchor="e")
    canvas.create_text(padding - 10, 10, text=f"{f2_range[1]:.2f}", anchor="e")
    ...
    for _, _, obj in lideres:
        ...
        canvas.create_oval(plot_x - 3, plot_y - 3, plot_x + 3, plot_y + 3, fill='red', outline='red')

# LÓGICA DE CONTROL DEL ALGORITMO
def iniciar_enjambre(event=None):
    global enjambre_creado
    if not enjambre_creado:
        enjambre.crear_enjambre(ancho_canva=ancho_canva-padding*2, alto_canva=alto_canva-padding*2)
        for p in enjambre.particulas:
            # Mapear las coordenadas al espacio visible del canvas
            x_canvas = map_value(p.x, 0, ancho_canva-padding*2, padding, ancho_canva-padding)
            y_canvas = map_value(p.y, 0, alto_canva-padding*2, alto_canva-padding, padding)
...
def iterar_algoritmo(event=None):
    ...
    if not enjambre_creado:
        ...
        return
    if iteracion_activa:
        return # Evita que se inicien múltiples bucles si se presiona 'W' varias veces
...
def ejecutar_un_paso():
    ...
    for i, p in enumerate(enjambre.particulas):
        vel_x_canvas = map_value(velocidades_x[i], -ancho_canva, ancho_canva, -padding, padding)
        vel_y_canvas = map_value(velocidades_y[i], -alto_canva, alto_canva, -padding, padding)
        decision_space_canvas.move(dibujo_particulas[i], vel_x_canvas, vel_y_canvas)
    ...
    for x, y, _ in enjambre.lideres:
        x_canvas = map_value(x, 0, ancho_canva-padding*2, padding, ancho_canva-padding)
        y_canvas = map_value(y, 0, alto_canva-padding*2, alto_canva-padding, padding)
        rect = decision_space_canvas.create_oval(
